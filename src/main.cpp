...
int port = 8888;                // The port number on which to start the HTTP server
const char *project_homepage_url = "https://askquickly.org";
...
rich_suggestions_json_array(vp_t& suggestions) {
    ...
    for (vp_t::iterator i = suggestions.begin(); i != suggestions.end(); ++i) {
        ...
        ret += " { \"value\": \"" + i->phrase + "\", \"score\": " + uint_to_string(i->weight) + 
...
void serve_request(client_t *client) {
    ...
    if (request_uri == "/askq/suggest/") {
        handle_suggest(client, url);
    }
    else if (request_uri == "/askq/import/") {
        handle_import(client, url);
    }
    else if (request_uri == "/askq/export/") {
        handle_export(client, url);
    }
    else if (request_uri == "/askq/stats/") {
...
show_usage(char *argv[]) {
    ...
    printf("Start AskQuickly Suggest.\n\n");
    printf("Optional arguments:\n\n");
    printf("-h, --help           This screen\n");
    printf("-f, --file=PATH      Path of the file containing the phrases\n");
    printf("-p, --port=PORT      TCP port on which to start suggest (default: 8888)\n");
...
main(int argc, char* argv[]) {
    ...
    cerr<<"INFO::Starting AskQuickly Suggest on port '"<<port<<"'\n";
    cerr<<"INFO::AskQuickly.org has no support for https on '"<<port<<"'\n";
